...
export function loadData() {
    ...
    const statsRow = database.query("SELECT * FROM stats WHERE id = 1").get();
    const wordsRows = database.query("SELECT * FROM words").all();
    return {
...
export function addWord(word) {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(word.word, word.word.toLowerCase(), word.meaning, word.phonetic, word.pos, word.example, word.example_cn, word.source, word.added, word.level, word.next_review, word.interval_days, word.error_count, word.review_count, JSON.stringify(word.history), word.prototype || "", word.variant || "", word.etymology || "");
}
export function getWord(word) {
    const database = getDb();
    const row = database.query("SELECT * FROM words WHERE word_lower = ?").get(word.toLowerCase());
...
export function updateWord(word, updates) {
    ...
    WHERE word_lower = ?
  `).run(updated.word, updated.word.toLowerCase(), updated.meaning, updated.phonetic, updated.pos, updated.example, updated.example_cn, updated.source, updated.added, updated.level, updated.next_review, updated.interval_days, updated.error_count, updated.review_count, JSON.stringify(updated.history), updated.prototype || "", updated.variant || "", updated.etymology || "", wordLower);
    return updated;
}
export function removeWord(word) {
    const database = getDb();
    const result = database.query("DELETE FROM words WHERE word_lower = ?").run(word.toLowerCase());
...
export function getDueWords() {
    const database = getDb();
    const today = new Date().toISOString().split("T")[0];
    const rows = database.query("SELECT * FROM words WHERE next_review <= ?").all(today);
    return rows.map(rowToWord);
}
...
export function updateWordEnrich(word, enrich) {
    const database = getDb();
    database.query(`
