...
export function calculateNextReview(currentLevel, feedback) {
    switch (feedback) {
        case "pass": {
...
export function processReviewFeedbacks(feedbacks) {
    const data = loadData();
    ...
    for (const { word, feedback } of feedbacks) {
        const existingWord = data.words.find(w => w.word.toLowerCase() === word.toLowerCase());
        if (!existingWord)
            continue;
        ...
        existingWord.interval_days = intervalDays;
