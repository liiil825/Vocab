 ...
 * 算法单元测试
 */
import { getToday, addDays, calculateNextReview, processReviewFeedbacks } from "../../dist/algorithm.js";
import { loadData, saveData, addWord, getWord, removeWord } from "../../dist/storage.js";
...
async function runTests() {
  ...
  addWord(testWord1);
  const passResult = processReviewFeedbacks([{ word: "testpass", feedback: "pass" }]);
  ...
  console.log("\n测试 5: processReviewFeedbacks (fail)");
  addWord({ ...testWord1, word: "testfail", level: 2 });
  const failResult = processReviewFeedbacks([{ word: "testfail", feedback: "fail" }]);
  ...
  console.log("\n测试 6: processReviewFeedbacks (fuzzy)");
  addWord({ ...testWord1, word: "testfuzzy", level: 2 });
  const fuzzyResult = processReviewFeedbacks([{ word: "testfuzzy", feedback: "fuzzy" }]);
  ...
  for (const w of ["word1", "word2", "word3"]) {
    addWord({ ...testWord1, word: w, level: 0, next_review: getToday() });
  }
  const batchResult = processReviewFeedbacks([
  ...
  console.log("\n测试 8: processReviewFeedbacks (不存在的词)");
  const nonExistResult = processReviewFeedbacks([{ word: "nonexistent", feedback: "pass" }]);
  ...
  data.streak = 5;
  writeTestData(data);
  addWord({ ...testWord1, word: "streaktest", level: 0, next_review: getToday() });
  const streakResult = processReviewFeedbacks([{ word: "streaktest", feedback: "pass" }]);
  ...
  data.streak = 10;
  writeTestData(data);
  const breakResult = processReviewFeedbacks([{ word: "streaktest", feedback: "pass" }]);
