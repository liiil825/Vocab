...
`vocab-trainer` is an MCP server providing spaced repetition vocabulary learning. It exposes 7 tools via the Model Context Protocol for OpenClaw agents to use.
...
storage.ts        # JSON file I/O, data path resolved at RUNTIME via getDataPath()
    ↓
~/.vocab-trainer/words.json  # Persistent storage
```
...
- `words.json` stores vocabulary with levels 0-5
